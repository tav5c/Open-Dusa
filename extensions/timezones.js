// ─────────────────────────────────────────────────────────────────────────────
// extensions/timezones.js — timezone tracking + meeting-time finder.
// Originally written by outcome (https://github.com/outcome) for his own bot;
// he stopped hosting and handed the code over — do whatever with it. Extracted
// from extensions/private.js into this standalone extension, polished with
// hybrid prefix commands (m.tz / m.timezone / m.tzset / ...), an optional
// `private` flag on the display slash commands, and a few extra region aliases.
// Data lives in data/ai/ with async write-through.
// Slash: /tzset, /tzremove, /tz, /tzbesttime, /tzis, /tzwhen
// Prefix (prefix + aliases from config): tz, timezone, tzset, tzremove,
//   tzbesttime, tzwhen, tzis
// ─────────────────────────────────────────────────────────────────────────────
import {
    ActionRowBuilder,
    ButtonBuilder,
...
export const manifest = {
    name: 'timezones',
    version: '2.0.0',
    author: 'outcome (https://github.com/outcome), extracted + polished',
    description: 'Timezone tracking + meeting-time finder (slash + prefix hybrid)',
    apiVersion: 1,
    slashCommands: [],
    permissions: [],
    dependencies: [],
...
// Data lives in data/ai/ alongside the other machine-written user data
// (never hand-edit).
// ─────────────────────────────────────────────────────────────────────────
...
for (const legacy of ['configs/timezones.json', 'timezones.json']) {
    if (!fs.existsSync(DATA_FILE) && fs.existsSync(legacy)) {
        try {
            fs.mkdirSync('data/ai', { recursive: true })
            fs.renameSync(legacy, DATA_FILE)
            console.log(`[Timezone] Moved ${legacy} into data/ai/`)
        } catch (e) {
...
function persistCache() {
    ...
    // Atomic tmp+rename: a kill mid-write must never leave a truncated
    // timezones.json behind (that failure mode wiped its neighbor file).
    try {
        const tmp = `${DATA_FILE}.tmp.${process.pid}`
        writeFileSync(tmp, JSON.stringify(_cache, null, 2))
        renameSync(tmp, DATA_FILE)
    } catch (err) {
        console.error('[timezone] persist failed:', err.message)
    }
}
...
const ABBR_RE =
    /^(UTC|ETC|EST|EDT|CST|CDT|MST|MDT|PST|PDT|AKS|AKD|HST|NST|NDT|AST|ADT|GMT|BST|WET|CET|EET|MSK|TRT|IST|PKT|BDT|ICT|SGT|MYT|PHT|HKT|JST|KST|GST|IRS|AFT|MMT|NPT|AES|AED|ACS|ACD|AWS|NZS|NZD|CHA|ART|BRT|CAT|EAT|WAT|SAS|SST|TOT|WIT|CIT|EIT|ET$|CT$|MT$|PT$)/
...
async function cmdTz(interaction) {
    ...
    // Optional `private` flag: answer ephemerally instead of in-channel.
    const eph = interaction.options.getBoolean?.('private') === true
    const EPH = eph ? { flags: MessageFlags.Ephemeral } : {}
...
async function cmdTzbesttime(interaction) {
    ...
    const scopeNote =
        taggedIds.length > 0 ? ` · scoped to ${taggedIds.length} user${taggedIds.length > 1 ? 's' : ''}` : ''
...
async function cmdTzwhen(interaction) {
    ...
    if (rawTZ) {
        ...
        if (!inv) {
            return interaction.reply({
                content:
                    '❌ No timezone saved for you. Set one with `/tzset` or pass a `timezone` to this command.',
                flags: MessageFlags.Ephemeral,
            })
        }
...
// ─────────────────────────────────────────────────────────────────────────────
// TZ DISPATCH, wired into the interaction chain in index.js.
...
function buildTzSlashCommands() {
    return [
        ...
        _tzCmd('tzis', 'Look up the current time for any timezone or location')
            .addStringOption((o) =>
                o.setName('location').setDescription('e.g. Malaysia, ET, UTC+8, Michigan').setRequired(true),
            )
            .addBooleanOption((o) => o.setName('private').setDescription('Only you see the answer')),
        _tzCmd('tzwhen', "Convert a time to everyone's timezone")
            .addStringOption((o) =>
                o.setName('time').setDescription('e.g. 9, 14, 9am, 9:42, 18:24:09').setRequired(true),
            )
            .addStringOption((o) =>
                o.setName('timezone').setDescription('Source timezone (default: your saved one)'),
            )
            .addUserOption((o) =>
                o.setName('user').setDescription('Show conversion for one specific user only'),
            )
            .addBooleanOption((o) => o.setName('private').setDescription('Only you see the answer')),
    ]
}
...
// ─────────────────────────────────────────────────────────────────────────────
// EXTENSION INTERFACE (manifest loader: init / handleInteraction / getSlashCommands)
// Prefix commands register on client.commands; slash definitions below.
// ─────────────────────────────────────────────────────────────────────────────
export async function init(client) {
    const has = (name, fn) => {
        if (!client.commands.has(name)) client.commands.set(name, fn)
    }
    // Hybrid prefix surface: m.tz / m.timezone for lookup, full set for the rest.
    has('tz', (msg, args) => prefixTz(msg, args))
    has('timezone', (msg, args) => prefixTz(msg, args))
    has('tzis', (msg, args) => prefixTzis(msg, args))
    has('tzset', (msg, args) => prefixTzset(msg, args))
    has('tzremove', (msg, args) => prefixTzremove(msg, args))
    has('tzbesttime', (msg, args) => prefixTzbesttime(msg, args))
    has('tzwhen', (msg, args) => prefixTzwhen(msg, args))
    console.log('[Timezone] Hybrid commands ready: tz/timezone/tzis/tzset/tzremove/tzbesttime/tzwhen')
}

export async function handleInteraction(interaction) {
    return handleTzInteraction(interaction)
}

export function getSlashCommands() {
    return buildTzSlashCommands()
}

// ─────────────────────────────────────────────────────────────────────────────
// AI LINK: timezone-aware answers for the chat pipeline (chat.js imports these).
// No Discord I/O here — pure data lookups so the AI layer stays testable.
// ─────────────────────────────────────────────────────────────────────────────
...
export function getLocalTimeLine(entry) {
...
// ─────────────────────────────────────────────────────────────────────────────
// PREFIX SHIM: adapts a prefix invocation into the interaction shape the
// cmd* handlers expect. Ephemeral flags are stripped (message replies can't
// be ephemeral); the `private` slash option reads as null -> public.
// Paginator buttons work identically — handleTzInteraction is customId-based.
// ─────────────────────────────────────────────────────────────────────────────
function shimInteraction(msg, { users = {}, strings = {}, bools = {} } = {}) {
    return {
        user: msg.author,
        ...
        isButton: () => false,
        options: {
            getUser: (n) => users[n] ?? null,
            getString: (n) => strings[n] ?? null,
            getBoolean: (n) => bools[n] ?? null,
        },
        // message replies reject `flags`, drop them silently.
        reply: (p) => {
            const { flags, ...rest } = p ?? {}
            return msg.reply(rest)
        },
        update: (p) => {
            const { flags, ...rest } = p ?? {}
            return msg.reply(rest)
...
const firstMentionedUser = (msg) => msg.mentions?.users?.first?.() ?? null

async function prefixTz(msg, args) {
    // m.tz / m.timezone — with a location acts like /tzis, otherwise like /tz.
    const rest = args.join(' ').trim()
    if (rest) return cmdTzis(shimInteraction(msg, { strings: { location: rest } }))
    const mentioned = firstMentionedUser(msg)
    return cmdTz(shimInteraction(msg, { users: mentioned ? { user: mentioned } : {} }))
}

async function prefixTzis(msg, args) {
    const rest = args.join(' ').trim()
    if (!rest)
        return msg.reply({
            content: 'Usage: `m.tzis <location>` — e.g. `m.tzis Malaysia`, `m.tzis UTC+8`',
            allowedMentions: { parse: [] },
        })
    return cmdTzis(shimInteraction(msg, { strings: { location: rest } }))
}
...
async function prefixTzbesttime(msg, args) {
    const ids = [...(msg.mentions?.users?.keys?.() ?? [])]
    return cmdTzbesttime(shimInteraction(msg, { strings: { users: ids.map((id) => `<@${id}>`).join(' ') } }))
}

async function prefixTzwhen(msg, args) {
    // m.tzwhen 9am [timezone] [@user] — first token that parses as time wins.
    const mentioned = firstMentionedUser(msg)
    const timeRx = /^(\d{1,2})(?::(\d{1,2}))?(?::(\d{1,2}))?\s*(am|pm)?$/i
    let time = null
    const rest = []
    for (const t of args) {
        if (!time && timeRx.test(t)) time = t
        else if (!/<@!?\d+>/.test(t)) rest.push(t)
    }
    if (!time)
        return msg.reply({
            content: 'Usage: `m.tzwhen 9am [timezone] [@user]`',
            allowedMentions: { parse: [] },
        })
    return cmdTzwhen(
        shimInteraction(msg, {
            strings: { time, timezone: rest.join(' ').trim() || null },
            users: mentioned ? { user: mentioned } : {},
        }),
    )
}
