...
-# ${sources.map((s) => `[${s.name}](<${s.url}>)`).join(' · ')}`
    ...
    async generateResponse({
        ...
        try {
            ...
            const ACTION_RE =
                /\b(ban|kick|mute|unmute|warn|purge|clear|mpurge|fpurge|delete|remove|lock|unlock|role|nickname|rename|avatar|av|pfp|banner|bn|announce|poll|thread|pin|unpin|slowmode|topic|react|emoji|movevc|dm|show|fetch|pull up|remind|reminder|remindme)\b/i
