...
const clr = {
    ...
    light_green: '\x1b[1;92m',
    light_yellow: '\x1b[1;93m',
    light_magenta: '\x1b[1;95m',
    light_cyan: '\x1b[1;96m',
    light_red: '\x1b[1;91m',
...
for (const file of extensionFiles) {
    try {
        ...
        const apiVer = mod.manifest?.apiVersion ?? 1
        if (apiVer > 1) {
            console.warn(`[System] Skipping ${file}: apiVersion ${apiVer} unsupported`)
            continue
        }
        ...
        console.log(
            `[System] Loaded extension: ${extName}${mod.manifest ? ` v${mod.manifest.version ?? '?'}` : ''}`,
        )
    } catch (e) {
        console.error(`[System] Failed to load extension ${file}:`, e.message)
    }
}
    ...
    if (globalThis._aiMemManagers) {
        for (const mgr of globalThis._aiMemManagers) {
            try {
                ...
                if (mgr.db?.open) {
                    mgr.db.pragma('wal_checkpoint(TRUNCATE)')
                    mgr.db.close()
                }
            } catch {}
        }
...
async function cmdAv(ctx, args) {
    ...
    const displayName = isRawUser ? (userObj.globalName ?? userObj.username) : member.displayName
    const color =
        !isRawUser && member.displayHexColor && member.displayHexColor !== '#000000'
            ? member.displayHexColor
            : 0x378add
    const avatarUrl = (member.displayAvatarURL ? member : userObj).displayAvatarURL({
        size: 1024,
        forceStatic: false,
    })
    await ctx.reply({
        embeds: [
            new EmbedBuilder()
                .setTitle(`👤 Avatar - ${displayName}`)
                .setImage(avatarUrl)
                .setColor(color)
                .setFooter({ text: 'Medusa • Server Avatar' }),
        ],
    })
}
...
async function cmdMav(ctx, args) {
    ...
    await ctx.reply({
        embeds: [
            new EmbedBuilder()
                .setTitle(`🖼️ Main Avatar - ${userObj.username}`)
                .setImage(userObj.displayAvatarURL({ size: 1024, forceStatic: false }))
                .setColor(0x378add),
        ],
    })
}
...
async function cmdBn(ctx, args) {
    ...
    try {
        ...
        if (data.banner)
            bannerUrl = `https://cdn.discordapp.com/guilds/${ctx.guild.id}/users/${member.id}/banners/${data.banner}.${data.banner.startsWith('a_') ? 'gif' : 'png'}?size=2048`
    } catch {}
    const user = !bannerUrl ? await client.users.fetch(member.id, { force: true }).catch(() => null) : null
    if (!bannerUrl && user) {
        bannerUrl = user.bannerURL({ size: 2048, forceStatic: false })
        if (bannerUrl) isFallback = true
    }
    const isRawUser = !member.joinedTimestamp && !member.user
    const embed = new EmbedBuilder()
    ...
    if (bannerUrl) {
        ...
        embed.setFooter({ text: isFallback ? '💡 Showing global banner' : 'Medusa • Server-Specific Banner' })
    } else {
        if (user?.hexAccentColor) embed.setColor(user.hexAccentColor)
        embed.setDescription('❌ No banner image found.')
    }
    await ctx.reply({ embeds: [embed] })
}
...
async function cmdMbn(ctx, args) {
    ...
    if (user.bannerURL()) {
        ...
        embed.setDescription('❌ No main banner set.')
    }
    if (ctx.deferred) await ctx.editReply({ embeds: [embed] })
...
async function cmdMenu(ctx) {
    ...
    const mkEmbed = (p) =>
        new EmbedBuilder()
            ...
            .setColor(0x378add)
            .setFooter({ text: `Page ${p + 1}/${pages.length} • n.snake (tav) • Project: Medusa` })
    const mkRow = (p) =>
    ...
    col.on('collect', async (i) => {
        if (i.user.id !== auth.id)
            return i.reply({ content: `Not yours. Use /menu.`, flags: MessageFlags.Ephemeral })
        if (i.customId === 'mn_close') {
            col.stop()
...
const SLASH_CMDS = [
    ...
    new SlashCommandBuilder()
        ...
        .addStringOption((o) => o.setName('duration').setDescription('e.g. 15m, 2h, 1d').setRequired(true))
        .addStringOption((o) => o.setName('reason').setDescription('Reason'))
    ...
    new SlashCommandBuilder()
        ...
        .setContexts(0)
        .addUserOption((o) => o.setName('member').setDescription('Target user').setRequired(true))
        .setDefaultMemberPermissions(PermissionFlagsBits.ModerateMembers),
    new SlashCommandBuilder()
        .setName('modlog')
...
for (const [name, ext] of client.extensions) {
    try {
        const mod = await import(`./extensions/${name}.js`)
        if (typeof mod.getSlashCommands === 'function') {
            const extCmds = mod.getSlashCommands()
            if (Array.isArray(extCmds) && extCmds.length) {
                SLASH_CMDS.push(...extCmds)
...
addCmd('help', async (msg, args) => {
    const name = args[0]
    if (!name)
        return msg.reply({ content: `Please provide a command name. Usage: \`${PREFIX}help <command>\`` })
    await msg.reply(
...
addCmd('setbotname', async (msg, args) => {
    ...
    try {
        ...
        await msg.reply(`✅ Global name changed.`)
    } catch (e) {
...
addCmd('setnickname', async (msg, args) => {
    ...
    if (
        ...
        msg.member.roles.highest.position <= msg.guild.members.me.roles.highest.position
    )
        return msg.reply('❌ You must have a role higher than mine to change my nickname!')
    try {
        await msg.guild.members.me.setNickname(args.join(' '))
        await msg.reply(`✅ Nickname changed.`)
    } catch (e) {
        await msg.reply(`❌ Failed: ${e.message}`)
    }
})
addCmd('createchan', async (msg, args) => {
    ...
    try {
        const ch = await msg.guild.channels.create({ name: args.slice(1).join('-') || 'new-channel', type })
        await msg.reply(`✅ Created <#${ch.id}>.`)
    } catch (e) {
        await msg.reply(`❌ Failed: ${e.message}`)
    }
})
...
addCmd('renameserver', async (msg, args) => {
    ...
    try {
        await msg.guild.setName(args.join(' '))
        await msg.reply(`✅ Server renamed.`)
    } catch (e) {
        await msg.reply(`❌ Failed: ${e.message}`)
    }
})
addCmd('addemoji', async (msg, args) => {
    ...
    try {
        const emoji = await msg.guild.emojis.create({ attachment: url, name })
        await msg.reply(`✅ Added emoji: ${emoji}`)
    } catch (e) {
        await msg.reply(`❌ Failed: ${e.message}`)
    }
})
addCmd('lockchannel', async (msg, args) => {
    ...
    try {
        await ch.permissionOverwrites.edit(roleId, { SendMessages: false })
        await msg.reply(`🔒 Locked ${ch}.`)
    } catch (e) {
        await msg.reply(`❌ Failed: ${e.message}`)
    }
})
addCmd('unlockchannel', async (msg, args) => {
    ...
    const roleId =
        args.find((a) => a.startsWith('<@&'))?.replace(/[<@&>]/g, '') || msg.guild.roles.everyone.id
    try {
        await ch.permissionOverwrites.edit(roleId, { SendMessages: null })
        await msg.reply(`🔓 Unlocked ${ch}.`)
    } catch (e) {
        await msg.reply(`❌ Failed: ${e.message}`)
    }
})
...
addCmd('addrole', async (msg, args) => {
    ...
    try {
        ...
        await msg.reply(`✅ Added **${role.name}** to ${target.user.username}.`)
    } catch (e) {
        await msg.reply(`❌ Failed: ${e.message}`)
    }
})
addCmd('removerole', async (msg, args) => {
    ...
    try {
        ...
        await msg.reply(`✅ Removed **${role.name}** from ${target.user.username}.`)
    } catch (e) {
        await msg.reply(`❌ Failed: ${e.message}`)
    }
})
...
client.once('clientReady', async () => {
    ...
    try {
        ...
        await registerAI(client, db, config)
    ...
    try {
        http.createServer((_, res) => {
            ...
            .listen(process.env.HEALTH_PORT || 1350)
            .unref()
        console.log(`[Health] Listening on port ${process.env.HEALTH_PORT || 1350}`)
    } catch {}
...
if (!token) {
    console.error('No token found in config.json or TOKEN env')
    process.exit(1)
}
client.login(token)
